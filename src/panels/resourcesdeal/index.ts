...
module.exports = Editor.Panel.define({
    ...
    methods: {
        ...
        applyPrefabFilter() {
            const filterValue = (this.$.prefabFilter as HTMLInputElement).value.trim().toLowerCase();
            ...
            // 如果有筛选关键词，进行筛选
            if (filterValue) {
                filteredRows = _originalPrefabData.filter(row => 
                    row.prefab.toLowerCase().includes(filterValue)
                );
            }
