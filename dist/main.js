...
async function preChangeImagesAndPrefabs(args) {
    ...
    try {
        ...
        Object.entries(preImageCache.duplicateGroups).forEach(([md5, paths]) => {
            ...
            removeImages.forEach(removePath => {
                const removeItem = preImageCache.removeImages[md5].find((item) => item.path === removePath);
                if (!removeItem)
                    return;
                const removeImageUuid = removeItem.info.uuid;
                const references = removeItem.references;
                // 修改每个引用该图片的预制体文件
                references.forEach((prefabPath) => {
                    try {
...
async function moveBgImages(args) {
    ...
    if (removedCount > 0) {
        ...
        operations.filter(op => op.src === op.dest).forEach(op => {
            no_ops.push(Object.assign(Object.assign({}, op), { reason: '大小写冲突处理后，源路径与目标路径相同' }));
        });
    }
    // 更新operations为过滤后的数组
    operations.length = 0;
...
function updateWithGlobalPrefabs(spriteFrameMaps_name, uuid2info, path2info) {
    ...
    const globalSpriteFrameToPrefabs = {};
    const addGlobalPair = (prefabRel, uuid) => {
        if (!uuid)
            return;
        if (!globalSpriteFrameToPrefabs[uuid])
            globalSpriteFrameToPrefabs[uuid] = new Set();
